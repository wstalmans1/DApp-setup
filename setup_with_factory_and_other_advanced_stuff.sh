...
async function generateDocs(hre: HardhatRuntimeEnvironment) {
  ...
  const builds: BuildInfo[] = buildInfoPaths.map(path => require(path))
  console.log(
    \`Generating docs from \${builds.length} build(s) (solidity-docgen)...\`
  )
...
pnpm --dir packages/contracts add -D \
  ...
  hardhat-deploy@^0.11.29 hardhat-docgen@^1.0.0 solidity-docgen@0.6.0-beta.36 \
